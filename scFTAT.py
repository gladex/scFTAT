...
class FFT_Mlp(layers.Layer):
    ...
    def __init__(self, dff_size,model_size, drop=0.01):
...
class SpectralGatingNetwork(layers.Layer):
    ...
    def __init__(self, model_size,maxlen,num_heads):
        super().__init__()
    ...
    def call(self, x, mask):                
        ...
        weighted_fft_x = tf.complex(self.complex_weight[:, :, :, 0], self.complex_weight[:, :, :, 1]) * fft_x
...
class FFT_Block(layers.Layer):
    ...
    def __init__(self, model_size,num_heads, dff_size, maxlen, drop_path=0.1):
        super().__init__()
...
class FFTEncoder(keras.Model):
    ...
    def __init__(self,num_layers,model_size,num_heads,dff_size,vocab_size,maxlen,drop_path=0.1):
    ...
    def call(self,x,training,padding_mask):
        ...
        for i in range(self.num_layers):
            x = self.FFTencoder_layers[i](x,training,padding_mask)
        return x
...
class MultiHeadAttention(keras.Model):
    ...
    def call(self, query, key, value, mask):
        ...
        cos_pos = K.repeat_elements(sinusoidal_pos[..., None, 1::2], rep=2, axis=-1)
        sin_pos = K.repeat_elements(sinusoidal_pos[..., None, ::2], rep=2, axis=-1)

        q2 = K.stack([-q[..., 1::2], q[..., ::2]], ndim)
        q2 = K.reshape(q2, K.shape(q))
        q = q * cos_pos + q2 * sin_pos
        k2 = K.stack([-k[..., 1::2], k[..., ::2]], ndim)
        k2 = K.reshape(k2, K.shape(k))
...
class MyAttn(layers.Layer):
    ...
    def __init__(self, model_size, act_ratio=0.25, act_fn=tf.nn.gelu, gate_fn=tf.nn.sigmoid):
...
class EncoderLayer(keras.layers.Layer):
    ...
    def __init__(self,model_size,num_heads,dff_size,maxlen,rate=0.1):
    ...
    def call(self,x,FFT_out,training,mask):
...
class Encoder(keras.Model):
    ...
    def call(self,FFT_out,x,training,padding_mask):
        x = self.embedding(x)+self.pos_embedding
        x = self.dropout(x,training=training)
        for i in range(self.num_layers):
            x = self.encoder_layers[i](x,FFT_out,training,padding_mask)
        return x
...
class Transformer(keras.Model):
    def __init__(self,num_layers,model_size,num_heads,dff_size,vocab_size,maxlen,training=True,rate=0.1):
    ...
    def call(self,all_inputs):
        ...
        FFT_out = self.FFTencoder(sources1,self.training,enc_padding_mask)
        enc_output = self.encoder(FFT_out,sources2,self.training,enc_padding_mask)
